...
namespace RandomEvents
{
    public class ItemStringParser
    {
        ...
        public static bool ParseItemStringReward(string itemString, Dictionary<PickupIndex, int> resolvedItems, List<PickupIndex> itemBlackList = null, ItemStringFormat format = ItemStringFormat.DEFAULT, int index = -1, int repeat = 1)
        {
            ...
            while (repeat > 0)
            {
                if (and)
                {
                    foreach (var entry in selectionList)
                    {
                        ...
                        {
                            success = ParseItemStringReward(replacements[entry.itemKey], subResolvedItems, itemBlackList, format, -1, entry.repeat);
                        }
                        else
                        {
        ...
        {
#if DEBUG
            string filePath = $"{RandomEvents.PluginName}_droptables.md";
